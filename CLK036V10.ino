...
int utc_hr_tens;
int utc_offset = 8;
int alarm_min = 56; // alarm variables
int alarm_hr = 17;
...
//  digitalWrite(TP1, HIGH);
  if  (UTC_On)
    {
    lc.setChar(0,7,segbuf[7],false); // display tens utc hours
    lc.setChar(0,6,segbuf[6],false); // display ones utc hours    
    }
  lc.setChar(0,5,segbuf[5],false); // display tens local hours

if (twelve_hr_mode and am_pm) // if am_pm = true then PM and light decimal point
    {
  lc.setChar(0,4,segbuf[4],true); // display ones hours with decimal point for PM
    }
  else
    {
  lc.setChar(0,4,segbuf[4],false); // display ones hours 
    }
...
// Initialize the segment buffers...
  ...
  // setup the clock
// Initialize DS3231
  Serial.println("CLK036V10 12/12/2020");
...
if (twelve_hr_mode)
  ...
  segbuf[6] = utc_hr_ones; // dt.second 0 to 59
  if (utc_hr_tens == 0) // blank the tens of hours if it is zero
   {
    segbuf[7] = 16; // dt.second blank
    }
  else
    {
    segbuf[7] = utc_hr_tens; // dt.second 0 to 59    
    }
if (!fast_update)
  {
   delay(1000);
